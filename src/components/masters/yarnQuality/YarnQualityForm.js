...
import { renderPartyQualityReference } from "./renderPartyQualityReference";
			...
			<Modal.Body>
				...
				<Row className="mt-3">
					<FieldArray
						...
						component={renderPartyQualityReference}
						isFetchingDropdown={isFetchingDropdown}
						errorsData={errors}
						metaData={meta}
						isViewOnly={isViewOnly}
...
export default reduxForm({
	...
	validate: values => {
		...
		isRequired(values?.yarn_type) && (errors.yarn_type = "Required");
		isRequired(values?.yarn_code) && (errors.yarn_code = "Required");
		isRequired(values?.hsn_code) && (errors.hsn_code = "Required");
		...
		values?.quality_developed?.forEach((quality, targetIndex) => {
			...
			isRequired(quality?.quality_id) && (qualityError.quality_id = "Required");
			isRequired(quality?.uom) && (qualityError.uom = "Required");
			isRequired(quality?.shade_id) && (qualityError.shade_id = "Required");
			isRequired(quality?.percentage) && (qualityError.percentage = "Required");
		...
		values?.party_shade_reference?.forEach((partyShade, targetIndex) => {
			...
			isRequired(partyShade.quality_reference) &&
		...
		let qualityReferenceList = {};
		values?.party_shade_reference?.forEach(psr => {
			if (psr?.party_id?.value) {
				if (psr?.party_id?.value in qualityReferenceList) {
					errors.party_shade_reference = {
						_error: "Duplicate Party Name selected",
					};
					return;
				} else {
					qualityReferenceList[psr?.party_id?.value] = true;
				}
			}
		});
