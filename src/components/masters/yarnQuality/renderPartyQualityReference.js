...
export const renderPartyQualityReference = ({
	fields,
	metaData,
	isFetchingDropdown,
	errorsData,
	...
	const columnsList = ["Supplier Name", "Party Quality Reference"];
		...
		<>
			...
			<div className="d-flex flex-column" style={{ overflowX: "auto" }}>
				<div className="d-flex flex-row align-items-center justify-content-around w-100 mb-2">
					<h4 className="text-start">Party Quality Reference</h4>
					{!isViewOnly && (
						<button
				...
				<div className="mx-auto">
					...
					{fields.map((party_shade_reference, index) => {
						const partyQualityReferenceTouched =
							metaData?.party_shade_reference?.[index]?.quality_reference
								?.touched;
						const partyQualityReferenceError =
							errorsData?.party_shade_reference?.[index]?.quality_reference;
							...
							<div key={index} className="text-nowrap">
								<div className="mb-1 subform-table-item">
									<Field
										...
										touched={
											metaData?.party_shade_reference?.[index]?.party_id
												?.touched
										}
										error={errorsData?.party_shade_reference?.[index]?.party_id}
								...
								<div className="mb-1 subform-table-item">
									<Tooltip
										title={
											partyQualityReferenceTouched && partyQualityReferenceError
												? partyQualityReferenceError
												: null
										}
									>
										<Field
